...
const MovieDetails = () => {
  ...
  useEffect(() => {
    ...
    const found = favourites.some(fav => fav._id === id);
  ...
  const toggleFavourite = () => {
    ...
    if (isFavourite) {
      ...
      favourites = favourites.filter(fav => fav._id !== id);
      localStorage.setItem('favouriteMovies', JSON.stringify(favourites));
      setIsFavourite(false);
    } else {
      // Add to favourites
      favourites.push(show.movie);
      localStorage.setItem('favouriteMovies', JSON.stringify(favourites));
      setIsFavourite(true);
    }
  };
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
        ...
        <div className='relative flex flex-col gp-3'>
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
            ...
            <button
              ...
              className={`p-2.5 rounded-full transition cursor-pointer active:scale-95 ${
                isFavourite ? 'bg-red-500' : 'bg-gray-700'
              }`}
            >
              <Heart
                className={`w-5 h-5 ${isFavourite ? 'fill-white text-white' : ''}`}
              />
            </button>
          </div>
        </div>

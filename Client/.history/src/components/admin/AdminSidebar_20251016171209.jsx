    ...
    <div className='h-[calc(100vh-64px)] md:flex flex-col items-center pt-8 max-w-13 md:w-60 border-r border-gray-300/30 text-sm'>
      ...
      <div className='w-full'>
        {adminNavlinks.map((link,index)=>(
             <NavLink key={index} className={({ isActive }) => `relative flex items-center max-md:justify-center gap-2 w-full py-2.5 min-md:pl-10 first:mt-6 text-gray-400 ${isActive && 'bg-primary/15 text-primary group'}`}>
                {( isActive )=>(
                    <>

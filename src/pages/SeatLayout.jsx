...
const SeatLayout = () => {
    ...
    const getShow = async () => {
        ...
        if(showData && dummyDateTimeData[date]){
            ...
            toast.error("Show or date not found")
        }
    }
    ...
    const handleSeatClick = (seatId) => {
        ...
        if(!selectedSeats.includes(seatId) && selectedSeats.length >= 5){
            return toast.error("You can only select five seats")
        }
                        ...
                        <button 
                            ...
                            className={`h-8 w-8 rounded border border-primary/60 cursor-pointer transition 
                                ${selectedSeats.includes(seatId) 
                                    ? "bg-red-100/40 scale-105" // very light red highlight only
                                    : "hover:bg-red-100/20 active:scale-95"}`} // subtle hover
                        >
                            {seatId}
                        </button>
                    )
    ...
    return (
        <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-10 md:pt-20'>
            {/* Available Timings */}
            <div className='w-60 bg-primary/10 border border-primary/20 rounded-lg py-8 h-max md:sticky md:top-10'>
                        ...
                        <div 
                            ...
                            className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition
                                ${selectedTime?.time === item.time 
                                    ? "bg-red-100/40" // very light red highlight only
                                    : "hover:bg-red-100/20"}`} // subtle hover
                        >
